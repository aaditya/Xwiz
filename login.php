...
$result = mysqli_query($link,"SELECT `id`,`pwd`,`full_name`,`user_name`,`approved`,`user_level` FROM users WHERE 
           $user_cond
			AND `banned` = '0'
...
$num = mysqli_num_rows($result);
  ...
  // Match row found with more than 1 results  - the user is authenticated. 
	...
	list($id,$pwd,$full_name,$uname,$approved,$user_level) = mysqli_fetch_row($result);
	...
	if(empty($err)){			
     ...
     // this sets session and logs user in  
	   ...
	   if(isset($_POST['remember'])){
				  ...
				  setcookie("uname",$_SESSION['uname'], time()+60*60*24*COOKIE_TIME_OUT, "/");
				   }
			$a = $_POST['token'];
			redirect("/parse?token=$a");
			die();
		 }
		}
		else
...
<table width="100%" border="0" cellspacing="0" cellpadding="5" class="main">
  ...
  <tr> 
    ...
    <h3 class="titlehdr"><a href="/" class="back"><i class="fa fa-arrow-circle-left fa-2x" style="padding-top:5px;"></i></a><p class="title"> Login Users</p></h3>  
	  <p>
	  <?php
	  /******************** ERROR MESSAGES*************************************************
      ...
      <form action="login.php" method="post" name="logForm" id="logForm" >
        <table width="65%" border="0" cellpadding="4" cellspacing="4" class="loginform">
          ...
          <tr> 
			...
			<input type="hidden" name="token" value="<?php echo $refer;?>" />
          </tr>
          <tr> 
